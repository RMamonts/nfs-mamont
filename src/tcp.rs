...
impl NFSTcpListener {
    ...
    pub async fn register_export<T>(&mut self, fs: T) -> Result<xdr::nfs3::fs_id, anyhow::Error>
    where
    ...
    pub async fn register_export_with_name<T, S>(
        ...
        fs: T,
        export_name: S,
    ) -> Result<xdr::nfs3::fs_id, anyhow::Error>
    where
        T: NFSFileSystem + Send + Sync + 'static,
        S: AsRef<str>,
    {
        ...
        if self.export_table.read().await.values().any(|entry| entry.export_name == export_name) {
            return Err(anyhow::anyhow!("Export name '{}' already exists", export_name));
        }
...
impl NFSTcp for NFSTcpListener {
    ...
    async fn set_mount_listener(
        ...
        self.export_table
            ...
            .await
            .get_mut(&fs_id)
            .ok_or_else(|| anyhow::anyhow!("Export ID {} not found", fs_id))?
