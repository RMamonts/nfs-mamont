...
fn nfs_call_frame(
    ...
    // Verifier AUTH_NONE
    push_u32(&mut payload, AUTH_NONE);
    push_u32(&mut payload, 0);
...
async fn parse_rejects_any_non_call_message_type() {
    ...
    let buf = vec![
        ...
        0x00, 0x00, 0x00, 0x00, // cred body len
        0x00, 0x00, 0x00, 0x00, // verifier flavor
        0x00, 0x00, 0x00, 0x00, // verifier body len
...
async fn parse_rejects_too_large_frame() {
    #[rustfmt::skip]
    let buf = vec![
        0x80, 0x02, 0x00, 0x01, // head with invalid frame size (DEFAULT_SIZE + 1)
    ];
    let socket = MockSocket::new(buf.as_slice());
    let alloc = MockAllocator::new(0);
    let mut parser = RpcParser::with_capacity(socket, alloc, 0x10);
