...
use crate::xdr;
use crate::xdr::mount::{dirpath, MountProgram, Mount_args};
...
) -> io::Result<Box<NFSv3_args>> {
    Ok(Box::new(match procedure {
        NFSProgram::NFSPROC3_NULL => NFSv3_args::NULL,
        NFSProgram::NFSPROC3_GETATTR => NFSv3_args::GETATTR(deserialize::<GETATTR3args>(input)?),
        NFSProgram::NFSPROC3_SETATTR => NFSv3_args::SETATTR(deserialize::<SETATTR3args>(input)?),
        NFSProgram::NFSPROC3_LOOKUP => NFSv3_args::LOOKUP(deserialize::<LOOKUP3args>(input)?),
        NFSProgram::NFSPROC3_ACCESS => NFSv3_args::ACCESS(deserialize::<ACCESS3args>(input)?),
        NFSProgram::NFSPROC3_READLINK => NFSv3_args::READLINK(deserialize::<READLINK3args>(input)?),
        NFSProgram::NFSPROC3_READ => NFSv3_args::READ(deserialize::<READ3args>(input)?),
        NFSProgram::NFSPROC3_WRITE => NFSv3_args::WRITE(deserialize::<WRITE3args>(input)?),
        NFSProgram::NFSPROC3_CREATE => NFSv3_args::CREATE(deserialize::<CREATE3args>(input)?),
        NFSProgram::NFSPROC3_MKDIR => NFSv3_args::MKDIR(deserialize::<MKDIR3args>(input)?),
        NFSProgram::NFSPROC3_SYMLINK => NFSv3_args::SYMLINK(deserialize::<SYMLINK3args>(input)?),
        NFSProgram::NFSPROC3_MKNOD => NFSv3_args::MKNOD(deserialize::<MKNOD3args>(input)?),
        NFSProgram::NFSPROC3_REMOVE => NFSv3_args::REMOVE(deserialize::<REMOVE3args>(input)?),
        NFSProgram::NFSPROC3_RMDIR => NFSv3_args::RMDIR(deserialize::<REMOVE3args>(input)?),
        NFSProgram::NFSPROC3_RENAME => NFSv3_args::RENAME(deserialize::<RENAME3args>(input)?),
        NFSProgram::NFSPROC3_LINK => NFSv3_args::LINK(deserialize::<LINK3args>(input)?),
        NFSProgram::NFSPROC3_READDIR => NFSv3_args::READDIR(deserialize::<READDIR3args>(input)?),
        NFSProgram::NFSPROC3_READDIRPLUS => {
            NFSv3_args::READDIRPLUS(deserialize::<READDIRPLUS3args>(input)?)
        }
        NFSProgram::NFSPROC3_FSSTAT => NFSv3_args::FSSTAT(deserialize::<FSSTAT3args>(input)?),
        NFSProgram::NFSPROC3_FSINFO => NFSv3_args::FSINFO(deserialize::<FSINFO3args>(input)?),
        NFSProgram::NFSPROC3_PATHCONF => NFSv3_args::PATHCONF(deserialize::<PATHCONF3args>(input)?),
        NFSProgram::NFSPROC3_COMMIT => NFSv3_args::COMMIT(deserialize::<COMMIT3args>(input)?),
        NFSProgram::INVALID => {
...
) -> io::Result<Box<Mount_args>> {
    Ok(Box::new(match procedure {
        MountProgram::MOUNTPROC3_NULL => Mount_args::NULL,
        MountProgram::MOUNTPROC3_MNT => Mount_args::MNT(deserialize::<dirpath>(input)?),
        MountProgram::MOUNTPROC3_DUMP => Mount_args::DUMP,
        MountProgram::MOUNTPROC3_UMNT => Mount_args::UMNT(deserialize::<dirpath>(input)?),
