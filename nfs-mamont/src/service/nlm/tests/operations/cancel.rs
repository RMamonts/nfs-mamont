...
async fn cancel_preserves_cookie() {
    ...
    svc.lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "alice", 100, 0))
        .await;
    svc.lock(None, make_lock_args_with_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "bob", 200, 0))
        .await;
...
async fn cancel_on_granted_lock_returns_granted() {
    let svc = NlmService::new();
    let res =
        svc.lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, 100, "alice", 100, 0)).await;
    assert_eq!(res.stat, Nlm4Stats::Granted);
