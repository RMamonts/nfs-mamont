...
async fn lock_grants_exclusive_lock() {
    ...
    let res = svc
        .lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 0))
        .await;
...
async fn lock_denies_conflicting_exclusive() {
    let svc = NlmService::new();
    svc.lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 0))
        .await;
    let res = svc
        .lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "other", 99, 1))
        .await;
...
async fn lock_allows_shared_overlapping() {
    let svc = NlmService::new();
    svc.lock(None, make_lock_args_without_block(FH_DEFAULT, false, 0, LOCK_WHOLE_LENGTH, "test", 42, 0))
        .await;
    let res =
        svc.lock(None, make_lock_args_without_block(FH_DEFAULT, false, 10, 20, "other", 99, 1)).await;
    assert_eq!(res.stat, Nlm4Stats::Granted);
}
...
async fn lock_denies_exclusive_against_shared() {
    ...
    svc.lock(None, make_lock_args_without_block(FH_DEFAULT, false, 0, LOCK_WHOLE_LENGTH, "test", 42, 0))
        .await;
    let res =
        svc.lock(None, make_lock_args_without_block(FH_DEFAULT, true, 10, 20, "other", 99, 1)).await;
    assert_eq!(res.stat, Nlm4Stats::Denied);
}
...
async fn lock_allows_different_files() {
    ...
    svc.lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 0))
        .await;
    let res = svc
        .lock(None, make_lock_args_without_block(FH_OTHER, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 1))
        .await;
    assert_eq!(res.stat, Nlm4Stats::Granted);
}
...
async fn lock_allows_non_overlapping_ranges() {
    ...
    let res = svc.lock(None, make_lock_args_without_block(FH_DEFAULT, true, 50, 50, "test", 42, 1)).await;
    assert_eq!(res.stat, Nlm4Stats::Granted);
}
...
async fn lock_preserves_cookie() {
    let svc = NlmService::new();
    let res = svc
        .lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 42))
        .await;
...
async fn lock_blocking_returns_blocked_on_conflict() {
    ...
    svc.lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 0))
        .await;
    let res = svc
        .lock(None, make_lock_args_with_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "other", 99, 1))
        .await;
...
async fn lock_blocking_still_grants_when_free() {
    let svc = NlmService::new();
    let res = svc
        .lock(None, make_lock_args_with_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 42))
        .await;
    assert_eq!(res.stat, Nlm4Stats::Granted);
}
...
async fn lock_same_owner_re_request_is_granted() {
    ...
    svc.lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 0))
        .await;
    let res = svc
        .lock(None, make_lock_args_without_block(FH_DEFAULT, true, 0, LOCK_WHOLE_LENGTH, "test", 42, 1))
        .await;
    assert_eq!(res.stat, Nlm4Stats::Granted);
}
...
async fn lock_unlock_lock_sequence_same_client() {
    let svc = NlmService::new();
    assert_eq!(
        svc.lock(None, make_lock_args_without_block(
            FH_DEFAULT,
            true,
            0,
            LOCK_WHOLE_LENGTH,
