...
/// Writes [`super::super::global::vfs::VfsPool`] responses and NLM GRANTED
/// callbacks to a network connection.
pub struct WriteTask<B: Buffer> {
    writehalf: OwnedWriteHalf,
    result_receiver: async_channel::Receiver<ProcReply<B>>,
    /// Channel carrying NLM GRANTED callbacks to be serialized and sent.
        ...
        loop {
            tokio::select! {
                reply = result_receiver.recv() => {
                    let Ok(reply) = reply else { break }; // all senders dropped
                ...
                cookie = granted_rx.recv() => {
                    ...
                    if let Err(e) = serializer.form_granted_call(cookie, credential, verifier).await {
                        error!(error=%e, "write task: failed to serialize/send GRANTED callback");
                    }
                }
            }
        }
