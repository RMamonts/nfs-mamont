...
/// Shared mutable reference to an optional VFS backend, enabling
/// deferred backend attachment after server start.
pub type SharedBackend<V> = Arc<RwLock<Option<Arc<V>>>>;
...
impl<B: Buffer + 'static> VfsPool<B> {
    ...
    /// - `shared_backend` --- shared optional filesystem implementation
    /// - `allocator` --- allocator used for read buffers
    ///
    /// # Returns
    ///
    /// A new [`VfsPool`] with the given number of workers.
    pub fn new<A, V>(num: NonZeroUsize, shared_backend: SharedBackend<V>, allocator: Arc<A>) -> Self
    where
        ...
        V: Vfs<B> + Send + Sync + 'static,
    {
        ...
        (0..num.get()).for_each(|_| {
            ...
            VfsTask::new(Arc::clone(&shared_backend), Arc::clone(&allocator), rx_clone).spawn();
        });
    ...
    V: vfs::Vfs<B> + Send + Sync + 'static,
{
    ...
    pub fn new(
        ...
        Self { shared_backend, allocator, command_receiver }
    }
    ...
    async fn run(self) {
        ...
        while let Ok((command, tx)) = command_receiver.recv().await {
            ...
            let backend: Option<Arc<V>> = self.shared_backend.read().unwrap().as_ref().cloned();

            let response = match backend {
                Some(backend) => match *proc {
                    ...
                    NfsArguments::Read(args) => {
                        let data_result = if args.count == 0 {
                            ...
                            self.allocator.allocate(requested_size).await.ok_or(vfs::read::Fail {
                ...
                None => Self::backend_not_ready_response(&proc),
            };
    ...
    /// Returns an [`NfsRes`] with [`vfs::Error::IO`] for every procedure,
    /// used when no backend has been attached yet.
    fn backend_not_ready_response(proc: &NfsArguments<B>) -> NfsRes<B> {
        match proc {
            NfsArguments::Null => NfsRes::Null,
            NfsArguments::GetAttr(_) => {
                NfsRes::GetAttr(Err(vfs::get_attr::Fail { error: vfs::Error::IO }))
            }
            NfsArguments::SetAttr(_) => NfsRes::SetAttr(Err(vfs::set_attr::Fail {
                error: vfs::Error::IO,
                wcc_data: vfs::WccData { before: None, after: None },
            })),
            NfsArguments::LookUp(_) => {
                NfsRes::LookUp(Err(vfs::lookup::Fail { error: vfs::Error::IO, dir_attr: None }))
            }
            NfsArguments::Access(_) => {
                NfsRes::Access(Err(vfs::access::Fail { error: vfs::Error::IO, object_attr: None }))
            }
            NfsArguments::ReadLink(_) => NfsRes::ReadLink(Err(vfs::read_link::Fail {
                error: vfs::Error::IO,
                symlink_attr: None,
            })),
            NfsArguments::Read(_) => {
                NfsRes::Read(Err(vfs::read::Fail { error: vfs::Error::IO, file_attr: None }))
            }
            NfsArguments::Write(_) => NfsRes::Write(Err(vfs::write::Fail {
                error: vfs::Error::IO,
                wcc_data: vfs::WccData { before: None, after: None },
            })),
            NfsArguments::Create(_) => NfsRes::Create(Err(vfs::create::Fail {
                error: vfs::Error::IO,
                wcc_data: vfs::WccData { before: None, after: None },
            })),
            NfsArguments::MkDir(_) => NfsRes::MkDir(Err(vfs::mk_dir::Fail {
                error: vfs::Error::IO,
                dir_wcc: vfs::WccData { before: None, after: None },
            })),
            NfsArguments::SymLink(_) => NfsRes::SymLink(Err(vfs::symlink::Fail {
                error: vfs::Error::IO,
                dir_wcc: vfs::WccData { before: None, after: None },
            })),
            NfsArguments::MkNod(_) => NfsRes::MkNod(Err(vfs::mk_node::Fail {
                error: vfs::Error::IO,
                dir_wcc: vfs::WccData { before: None, after: None },
            })),
            NfsArguments::Remove(_) => NfsRes::Remove(Err(vfs::remove::Fail {
                error: vfs::Error::IO,
                dir_wcc: vfs::WccData { before: None, after: None },
            })),
            NfsArguments::RmDir(_) => NfsRes::RmDir(Err(vfs::rm_dir::Fail {
                error: vfs::Error::IO,
                dir_wcc: vfs::WccData { before: None, after: None },
            })),
            ...
            NfsArguments::Link(_) => NfsRes::Link(Err(vfs::link::Fail {
                error: vfs::Error::IO,
                file_attr: None,
                dir_wcc: vfs::WccData { before: None, after: None },
            })),
            NfsArguments::ReadDir(_) => {
                NfsRes::ReadDir(Err(vfs::read_dir::Fail { error: vfs::Error::IO, dir_attr: None }))
            }
            NfsArguments::ReadDirPlus(_) => NfsRes::ReadDirPlus(Err(vfs::read_dir_plus::Fail {
                error: vfs::Error::IO,
                dir_attr: None,
            })),
            NfsArguments::FsStat(_) => {
                NfsRes::FsStat(Err(vfs::fs_stat::Fail { error: vfs::Error::IO, root_attr: None }))
            }
            NfsArguments::FsInfo(_) => {
                NfsRes::FsInfo(Err(vfs::fs_info::Fail { error: vfs::Error::IO, root_attr: None }))
            }
            NfsArguments::PathConf(_) => NfsRes::PathConf(Err(vfs::path_conf::Fail {
                error: vfs::Error::IO,
                file_attr: None,
            })),
            NfsArguments::Commit(_) => NfsRes::Commit(Err(vfs::commit::Fail {
                error: vfs::Error::IO,
