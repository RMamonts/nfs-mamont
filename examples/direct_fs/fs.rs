...
const READ_DIR_PREF: u32 = 64 * 1024;
const DIRECT_IO_ALIGNMENT: usize = 4096;
const DIRECT_IO_MIN_BYTES: usize = 128 * 1024;
...
impl MirrorFS {
    ...
    pub fn new(root: PathBuf) -> Self {
        ...
        let generation =
            SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or(Duration::ZERO).as_nanos()
                as u64;
        let direct_io = std::env::var("NFS_DIRECTFS_ODIRECT")
            .map(|v| v != "0" && !v.eq_ignore_ascii_case("false"))
            .unwrap_or(true);
        Self { fsmap: RwLock::new(FsMap::new(root)), generation, direct_io }
    }
    ...
    fn should_try_direct_io(&self, offset: u64, size: usize) -> bool {
        self.direct_io
            && size >= DIRECT_IO_MIN_BYTES
            && offset % DIRECT_IO_ALIGNMENT as u64 == 0
            && size % DIRECT_IO_ALIGNMENT == 0
    }

    fn open_for_read(path: &Path, direct: bool) -> std::io::Result<std::fs::File> {
    ...
    fn open_for_write(path: &Path, direct: bool) -> std::io::Result<std::fs::File> {
        let mut options = std::fs::OpenOptions::new();
    ...
    fn read_at_blocking(
        ...
        while read_total < dest.len() {
            let n = unsafe {
                libc::pread(
                    file.as_raw_fd(),
                    dest[read_total..].as_mut_ptr().cast(),
                    dest.len() - read_total,
    ...
    fn write_all_at_blocking(file: &std::fs::File, offset: u64, src: &[u8]) -> std::io::Result<()> {
    ...
    fn sync_file(file: &std::fs::File, stable: write::StableHow) -> std::io::Result<()> {
        match stable {
