...
async fn fs_info_returns_server_limits() {
    ...
    assert_eq!(result.read_max, 1024 * 1024);
    assert_eq!(result.write_max, 1024 * 1024);
    assert_eq!(result.read_dir_pref, 32 * 1024);
...
async fn fs_stat_returns_zero_counters() {
    ...
    assert!(result.free_bytes >= result.available_bytes);
    assert!(result.total_files >= result.free_files);
    assert!(result.free_files >= result.available_files);
